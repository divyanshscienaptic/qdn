...
import com.hp.QppColumbia.services.exception.DuplicateHomologatedNameException;
import com.hp.QppColumbia.services.repository.*;
...
public class DataServiceImpl implements com.hp.QppColumbia.services.service.DataService {
    ...
    @Autowired
    AccountSummaryPeruRepo accountSummaryPeruRepo;

    @Autowired
    AccountSummaryChileRepo accountSummaryChileRepo;

    @Autowired
    AccountSummaryArgentinaRepo accountSummaryArgentinaRepo;

    @Autowired
    AccountSummaryHistoryDetailsPeruRepo accountSummaryHistoryDetailsPeruRepo;

    @Autowired
    AccountSummaryHistoryDetailsArgentinaRepo accountSummaryHistoryDetailsArgentinaRepo;

    @Autowired
    AccountSummaryHistoryDetailsChileRepo accountSummaryHistoryDetailsChileRepo;
    ...
    private Boolean isHomologatedNameDuplicate(String name){
        return (accountSummaryRepo.findAllRecordsByHN(name).size() > 0) ? Boolean.TRUE : Boolean.FALSE;
    }

    @Override
    public List<AccountSummaryDetailsDao> addSellerToChile(List<AddAccountSummaryDAO> sellerDetails){
        List<AccountSummaryDetailsDao> sellers = new ArrayList<>();
        for(AddAccountSummaryDAO seller : sellerDetails){
            if(isHomologatedNameDuplicate(seller.getHomologatedName())){
                throw new DuplicateHomologatedNameException("Homologated name '"+seller.getHomologatedName()+"' already exists");
            }
            AccountSummaryDetailsDao newSeller = modelMapper.map(seller, AccountSummaryDetailsDao.class);
            sellers.add(newSeller);
        }
        return accountSummaryRepo.saveAll(sellers);
    }
    ...
    public boolean editRecord(EditASDetails editRecord) {
        // TODO Auto-generated method stub
        Optional<AccountSummaryDetailsDao> as = accountSummaryRepo
        ...
        if (!as.isPresent()) {
            ...
            String mpa = editRecord.getMpa();
            if (mpa != null && !mpa.trim().equals(asddao.getMpa())) {
                asddao.setMpa(mpa);
                asdh.setMpa(mpa);
                asdh.setMpaFlag("true");
            } else {
                asdh.setMpa(mpa);
                asdh.setMpaFlag("false");
            }

            String largeFormatStatus = editRecord.getLargeFormatStatus();
            if (largeFormatStatus != null && !largeFormatStatus.trim().equals(asddao.getLargeFormatStatus())) {
                asddao.setLargeFormatStatus(largeFormatStatus);
                asdh.setLargeFormatStatus(largeFormatStatus);
                asdh.setLargeFormatStatusFlag("true");
            } else {
                asdh.setLargeFormatStatus(largeFormatStatus);
                asdh.setLargeFormatStatusFlag("false");
            }
    ...
    public List<AccountSummaryDetailsDao> getAllFilteredRecords(AccountSummaryFilterDetails searchByFields) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getHomologatedName())) {
            ...
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo
                    .findAllRecordsLikeHN(searchByFields.getHomologatedName());
        ...
        if (!StringUtils.isEmpty(searchByFields.getLocationId())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo
                    .findAllRecordsLikeLocationId(searchByFields.getLocationId());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getRazonSocial())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo
                    .findAllRecordsLikeRazonSocial(searchByFields.getRazonSocial());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getTaxId())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsLikeTaxId(searchByFields.getTaxId());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getBpsStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsByBpsStatus(searchByFields.getBpsStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getHpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsByHpsStatus(searchByFields.getHpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getOpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsByOpsStatus(searchByFields.getOpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getMpa())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsByMPA(searchByFields.getMpa());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getLargeFormatStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsDao> udd = accountSummaryRepo.findAllRecordsByLargeFormatStatus(searchByFields.getLargeFormatStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
    ...
    public ASFilterResponse getFilterDetails() {
        ...
        List<String> mpa = accountSummaryRepo.findAllMPA();
        if (mpa != null && !mpa.contains(null))
            mpa.sort(Comparator.naturalOrder());
        ufdr.setMpa(mpa);

        List<String> largeFormatStatus = accountSummaryRepo.findAllLargeFormatStatus();
        if (largeFormatStatus != null && !largeFormatStatus.contains(null))
            largeFormatStatus.sort(Comparator.naturalOrder());
        ufdr.setLargeFormatStatus(largeFormatStatus);

        List<String> hpsHardwareStatus = accountSummaryRepo.findAllHpsHardwareStatus();
        if (hpsHardwareStatus != null && !hpsHardwareStatus.contains(null))
            hpsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setHpsHardwareStatus(hpsHardwareStatus);

        List<String> opsHardwareStatus = accountSummaryRepo.findAllOpsHardwareStatus();
        if (opsHardwareStatus != null && !opsHardwareStatus.contains(null))
            opsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setOpsHardwareStatus(opsHardwareStatus);

        List<String> bpsStatus = accountSummaryRepo.findAllBpsStatus();
        if (bpsStatus != null && !bpsStatus.contains(null))
            bpsStatus.sort(Comparator.naturalOrder());
        ufdr.setBpsStatus(bpsStatus);
            ...
            if (k == 0) {
                ...
                for (Cell cell : row) {
                    ...
                    if (i == 0 && !getValueOf(cell).equals("homologatedName")) {
                        ...
                        isValid = false;
                    } else if (i == 8 && !getValueOf(cell).equals("qdnApplicationForm")) {
                        isValid = false;
                    } else if (i == 9 && !getValueOf(cell).equals("suppliesStatus")) {
                        isValid = false;
                    } else if (i == 10 && !getValueOf(cell).equals("largeFormatStatus")){
                        isValid = false;
                    } else if (i == 11 && !getValueOf(cell).equals("hpsHardwareStatus")) {
                        isValid = false;
                    } else if (i == 12 && !getValueOf(cell).equals("opsHardwareStatus")) {
                        isValid = false;
                    } else if (i == 13 && !getValueOf(cell).equals("bpsStatus")) {
                        isValid = false;
                    } else if (i == 14 && !getValueOf(cell).equals("mpa")) {
                        isValid = false;
                    } else if (i == 15 && !getValueOf(cell).equals("hpContactName")) {
                        isValid = false;
                    } else if (i == 16 && !getValueOf(cell).equals("hpContactEmail")) {
                        isValid = false;
                    } else if (i == 17 && !getValueOf(cell).equals("resellerContactName")) {
                        isValid = false;
                    } else if (i == 18 && !getValueOf(cell).equals("resellerContactEmail")) {
                        isValid = false;
                    } else if (i == 19 && !getValueOf(cell).equals("resellerContactPhone")) {
                        isValid = false;
                    } else if (i == 20 && !getValueOf(cell).equals("resellerWebAddress")) {
                        isValid = false;
                    } else if (i == 21 && !getValueOf(cell).equals("resellerPhysicalAddress")) {
                        isValid = false;
                    } else if (i == 22 && !getValueOf(cell).equals("cityOrDistrict")) {
                        isValid = false;
                    } else if (i == 23 && !getValueOf(cell).equals("stateOrRegion")) {
                        isValid = false;
                    } else if (i == 24 && !getValueOf(cell).equals("zipCode")) {
                        isValid = false;
                    } else if (i == 25 && !getValueOf(cell).equals("membershipForPc")) {
                        isValid = false;
                    } else if (i == 26 && !getValueOf(cell).equals("membershipForPrint")) {
                        isValid = false;
                    } else if (i == 27 && !getValueOf(cell).equals("amplifyMembershipForSupplies")) {
                        isValid = false;
                    } else if (i == 28 && !getValueOf(cell).equals("qdnStartDate")) {
                        isValid = false;
                    } else if (i == 29 && !getValueOf(cell).equals("qdnEndDate")) {
                        isValid = false;
                    } else if (i == 30 && !getValueOf(cell).equals("commentsORNotes")) {
                        isValid = false;
                    }
                    if (!isValid) {
                        k++;
                ...
                for (Cell cell : row) {
                    ...
                    if (cell.getColumnIndex() == 10) {
                        asddao.setLargeFormatStatus(getValueOf(cell));
                        asdh.setLargeFormatStatus(getValueOf(cell));
                        i++;
                        k++;
                        continue;
                    }
                    ...
                    if (cell.getColumnIndex() == 14) {
                        asddao.setMpa(getValueOf(cell));
                        asdh.setMpa(getValueOf(cell));
                        i++;
                        k++;
                        continue;
                    }
    ...
    public List<AccountSummaryDetailsPeruDao> getAllPeruRecords() {
        return accountSummaryPeruRepo.findAllRecords();
    }

    @Override
    public List<AccountSummaryDetailsPeruDao> getAllFilteredRecordsForPeru(AccountSummaryFilterDetails searchByFields) {
        // TODO Auto-generated method stub
        boolean checkSearchStatus = false;
        List<AccountSummaryDetailsPeruDao> allRevenue = new ArrayList<>();
        boolean andflag = false;
        if (!StringUtils.isEmpty(searchByFields.getHomologatedName())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo
                    .findAllRecordsLikeHN(searchByFields.getHomologatedName());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
        ...
        if (!StringUtils.isEmpty(searchByFields.getTaxId())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsLikeTaxId(searchByFields.getTaxId());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getBpsStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsByBpsStatus(searchByFields.getBpsStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getHpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsByHpsStatus(searchByFields.getHpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getOpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsByOpsStatus(searchByFields.getOpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getMpa())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsByMPA(searchByFields.getMpa());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getLargeFormatStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsPeruDao> udd = accountSummaryPeruRepo.findAllRecordsByLargeFormatStatus(searchByFields.getLargeFormatStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!checkSearchStatus) {
            return getAllPeruRecords();
        } else {
            return allRevenue.stream()
                    .collect(Collectors.collectingAndThen(toCollection(
                                    () -> new TreeSet<>(Comparator.comparingInt(AccountSummaryDetailsPeruDao::getUnique_id))),
                            ArrayList::new));
        }
    }
    ...
    public boolean editRecordForPeru(EditASDetails editRecord) {
        Optional<AccountSummaryDetailsPeruDao> as = accountSummaryPeruRepo
                .findRecordDetailsNew(editRecord.getHomologatedName());
        if (!as.isPresent()) {
            return false;
        } else {
            AccountSummaryDetailsPeruDao asddao = as.get();
            AccountSummaryDetailsHistoryPeruDao asdh = new AccountSummaryDetailsHistoryPeruDao();
            ...
            accountSummaryPeruRepo.save(asddao);
            accountSummaryHistoryDetailsPeruRepo.save(asdh);
            return true;
        }
    }
    ...
    public List<AccountSummaryDetailsHistoryPeruDao> getAllHistoryRecordsForPeru(String homologatedName) {
        return accountSummaryHistoryDetailsPeruRepo.findAllRecords(homologatedName);
    }

    @Override
    public List<AccountSummaryDetailsHistoryPeruDao> getAllTheHistoryRecordsForPeru() {
        return accountSummaryHistoryDetailsPeruRepo.findAllTheRecords();
    }
    ...
    public ASFilterResponse getFilterDetailsForPeru() {
        ...
        List<String> markets = accountSummaryPeruRepo.findallHM();
        if (markets != null && !markets.contains(null))
            markets.sort(Comparator.naturalOrder());
        ufdr.setHomologatedName(markets);

        List<String> pNames = accountSummaryPeruRepo.findallRS();
        if (pNames != null && !pNames.contains(null))
            pNames.sort(Comparator.naturalOrder());
        ufdr.setRazonSocial(pNames);

        List<String> siteNames = accountSummaryPeruRepo.findallTI();
        if (siteNames != null && !siteNames.contains(null))
            siteNames.sort(Comparator.naturalOrder());
        ufdr.setTaxId(siteNames);

        List<String> allTypes = accountSummaryPeruRepo.findallLocationId();
        if (allTypes != null && !allTypes.contains(null))
            allTypes.sort(Comparator.naturalOrder());
        ufdr.setLocationId(allTypes);

        List<String> mPId = accountSummaryPeruRepo.findallSuppliesStatus();
        if (mPId != null && !mPId.contains(null))
            mPId.sort(Comparator.naturalOrder());
        ufdr.setSuppliesStatus(mPId);

        List<String> mpa = accountSummaryPeruRepo.findAllMPA();
        if (mpa != null && !mpa.contains(null))
            mpa.sort(Comparator.naturalOrder());
        ufdr.setMpa(mpa);

        List<String> largeFormatStatus = accountSummaryPeruRepo.findAllLargeFormatStatus();
        if (largeFormatStatus != null && !largeFormatStatus.contains(null))
            largeFormatStatus.sort(Comparator.naturalOrder());
        ufdr.setLargeFormatStatus(largeFormatStatus);

        List<String> hpsHardwareStatus = accountSummaryPeruRepo.findAllHpsHardwareStatus();
        if (hpsHardwareStatus != null && !hpsHardwareStatus.contains(null))
            hpsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setHpsHardwareStatus(hpsHardwareStatus);

        List<String> opsHardwareStatus = accountSummaryPeruRepo.findAllOpsHardwareStatus();
        if (opsHardwareStatus != null && !opsHardwareStatus.contains(null))
            opsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setOpsHardwareStatus(opsHardwareStatus);

        List<String> bpsStatus = accountSummaryPeruRepo.findAllBpsStatus();
        if (bpsStatus != null && !bpsStatus.contains(null))
            bpsStatus.sort(Comparator.naturalOrder());
        ufdr.setBpsStatus(bpsStatus);
    ...
    public String uploadRecordsForPeru(MultipartFile attachment, String submittedBy, String submittedOn, String comment) throws IOException {
        ...
        Workbook workbook = new XSSFWorkbook(inputStream);
        Sheet sheet = workbook.getSheetAt(0);
        log.info("IN UPLOAD FOR PERU");
        Map<Integer, List<String>> data = new HashMap<>();
        int k = 0;
        for (Row row : sheet) {
            /* data.put(i, new ArrayList<String>()); */
            int i = 0;
            AccountSummaryDetailsPeruDao asddao = new AccountSummaryDetailsPeruDao();
            boolean repeat = false;
            AccountSummaryDetailsHistoryPeruDao asdh = new AccountSummaryDetailsHistoryPeruDao();
            if (k == 0) {
                boolean isValid = true;
                ...
                for (Cell cell : row) {
                    ...
                    if (cell.getColumnIndex() == 0) {
                        if (accountSummaryPeruRepo.findAllRecordsByHN(getValueOf(cell)).size() != 0) {
                            repeat = true;
                            k++;
                            break;
                        }
                    ...
                    if (cell.getColumnIndex() == 28) {
                        asddao.setQdnStartDate(getValueOf(cell));
                        asdh.setQdnStartDate(getValueOf(cell));
                        log.info("VALUE FOR PERU START"+getValueOf(cell));
                        i++;
                        k++;
                        continue;
                    }
    ...
    public AccountSummaryDetailsPeruDao getRecordDetailsForPeru(String homologatedName) {
        // TODO Auto-generated method stub
        return accountSummaryPeruRepo.findRecordDetails(homologatedName);
    }
    ...
    public List<AccountSummaryDetailsArgentinaDao> getAllArgentinaRecords() {
        return accountSummaryArgentinaRepo.findAllRecords();
    }

    @Override
    public boolean editRecordForArgentina(EditASDetails editRecord) {
        Optional<AccountSummaryDetailsArgentinaDao> as = accountSummaryArgentinaRepo
                .findRecordDetailsNew(editRecord.getHomologatedName());
        if (!as.isPresent()) {
            return false;
        } else {
            AccountSummaryDetailsArgentinaDao asddao = as.get();
            AccountSummaryDetailsHistoryArgentinaDao asdh = new AccountSummaryDetailsHistoryArgentinaDao();
            ...
            accountSummaryArgentinaRepo.save(asddao);
            accountSummaryHistoryDetailsArgentinaRepo.save(asdh);
            return true;
        }
    }
    ...
    public List<AccountSummaryDetailsHistoryArgentinaDao> getAllHistoryRecordsForArgentina(String homologatedName) {
        return accountSummaryHistoryDetailsArgentinaRepo.findAllRecords(homologatedName);
    }

    @Override
    public List<AccountSummaryDetailsHistoryArgentinaDao> getAllTheHistoryRecordsForArgentina() {
        return accountSummaryHistoryDetailsArgentinaRepo.findAllTheRecords();
    }
    ...
    public ASFilterResponse getFilterDetailsForArgentina() {
        ...
        List<String> markets = accountSummaryArgentinaRepo.findallHM();
        if (markets != null && !markets.contains(null))
            markets.sort(Comparator.naturalOrder());
        ufdr.setHomologatedName(markets);

        List<String> pNames = accountSummaryArgentinaRepo.findallRS();
        if (pNames != null && !pNames.contains(null))
            pNames.sort(Comparator.naturalOrder());
        ufdr.setRazonSocial(pNames);

        List<String> siteNames = accountSummaryArgentinaRepo.findallTI();
        if (siteNames != null && !siteNames.contains(null))
            siteNames.sort(Comparator.naturalOrder());
        ufdr.setTaxId(siteNames);

        List<String> allTypes = accountSummaryArgentinaRepo.findallLocationId();
        if (allTypes != null && !allTypes.contains(null))
            allTypes.sort(Comparator.naturalOrder());
        ufdr.setLocationId(allTypes);

        List<String> mPId = accountSummaryArgentinaRepo.findallSuppliesStatus();
        if (mPId != null && !mPId.contains(null))
            mPId.sort(Comparator.naturalOrder());
        ufdr.setSuppliesStatus(mPId);

        List<String> mpa = accountSummaryArgentinaRepo.findAllMPA();
        if (mpa != null && !mpa.contains(null))
            mpa.sort(Comparator.naturalOrder());
        ufdr.setMpa(mpa);

        List<String> largeFormatStatus = accountSummaryArgentinaRepo.findAllLargeFormatStatus();
        if (largeFormatStatus != null && !largeFormatStatus.contains(null))
            largeFormatStatus.sort(Comparator.naturalOrder());
        ufdr.setLargeFormatStatus(largeFormatStatus);

        List<String> hpsHardwareStatus = accountSummaryArgentinaRepo.findAllHpsHardwareStatus();
        if (hpsHardwareStatus != null && !hpsHardwareStatus.contains(null))
            hpsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setHpsHardwareStatus(hpsHardwareStatus);

        List<String> opsHardwareStatus = accountSummaryArgentinaRepo.findAllOpsHardwareStatus();
        if (opsHardwareStatus != null && !opsHardwareStatus.contains(null))
            opsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setOpsHardwareStatus(opsHardwareStatus);

        List<String> bpsStatus = accountSummaryArgentinaRepo.findAllBpsStatus();
        if (bpsStatus != null && !bpsStatus.contains(null))
            bpsStatus.sort(Comparator.naturalOrder());
        ufdr.setBpsStatus(bpsStatus);
    ...
    public String uploadRecordsForArgentina(MultipartFile attachment, String submittedBy, String submittedOn, String comment) throws IOException {
        ...
        for (Row row : sheet) {
            /* data.put(i, new ArrayList<String>()); */
            int i = 0;
            AccountSummaryDetailsArgentinaDao asddao = new AccountSummaryDetailsArgentinaDao();
            boolean repeat = false;
            AccountSummaryDetailsHistoryArgentinaDao asdh = new AccountSummaryDetailsHistoryArgentinaDao();
            if (k == 0) {
                boolean isValid = true;
                ...
                for (Cell cell : row) {
                    ...
                    if (cell.getColumnIndex() == 0) {
                        if (accountSummaryArgentinaRepo.findAllRecordsByHN(getValueOf(cell)).size() != 0) {
                            repeat = true;
                            k++;
                            break;
                        }
    ...
    public AccountSummaryDetailsArgentinaDao getRecordDetailsForArgentina(String homologatedName) {
        // TODO Auto-generated method stub
        return accountSummaryArgentinaRepo.findRecordDetails(homologatedName);
    }

    @Override
    public List<AccountSummaryDetailsArgentinaDao> getAllFilteredRecordsForArgentina(AccountSummaryFilterDetails searchByFields) {
        // TODO Auto-generated method stub
        boolean checkSearchStatus = false;
        List<AccountSummaryDetailsArgentinaDao> allRevenue = new ArrayList<>();
        boolean andflag = false;
        if (!StringUtils.isEmpty(searchByFields.getHomologatedName())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo
                    .findAllRecordsLikeHN(searchByFields.getHomologatedName());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
        ...
        if (!StringUtils.isEmpty(searchByFields.getTaxId())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsLikeTaxId(searchByFields.getTaxId());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getBpsStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsByBpsStatus(searchByFields.getBpsStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getHpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsByHpsStatus(searchByFields.getHpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getOpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsByOpsStatus(searchByFields.getOpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getMpa())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsByMPA(searchByFields.getMpa());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getLargeFormatStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsArgentinaDao> udd = accountSummaryArgentinaRepo.findAllRecordsByLargeFormatStatus(searchByFields.getLargeFormatStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!checkSearchStatus) {
            return getAllArgentinaRecords();
        } else {
            return allRevenue.stream()
                    .collect(Collectors.collectingAndThen(toCollection(
                                    () -> new TreeSet<>(Comparator.comparingInt(AccountSummaryDetailsArgentinaDao::getUnique_id))),
                            ArrayList::new));
        }
    }
    ...
    public List<AccountSummaryDetailsChileDao> getAllChileRecords() {
        return accountSummaryChileRepo.findAllRecords();
    }

    @Override
    public boolean editRecordForChile(EditASDetails editRecord) {
        Optional<AccountSummaryDetailsChileDao> as = accountSummaryChileRepo
                .findRecordDetailsNew(editRecord.getHomologatedName());
        if (!as.isPresent()) {
            return false;
        } else {
            AccountSummaryDetailsChileDao asddao = as.get();
            AccountSummaryDetailsHistoryChileDao asdh = new AccountSummaryDetailsHistoryChileDao();
            ...
            accountSummaryChileRepo.save(asddao);
            accountSummaryHistoryDetailsChileRepo.save(asdh);
            return true;
        }
    }
    ...
    public List<AccountSummaryDetailsHistoryChileDao> getAllHistoryRecordsForChile(String homologatedName) {
        return accountSummaryHistoryDetailsChileRepo.findAllRecords(homologatedName);
    }

    @Override
    public List<AccountSummaryDetailsHistoryChileDao> getAllTheHistoryRecordsForChile() {
        return accountSummaryHistoryDetailsChileRepo.findAllTheRecords();
    }
    ...
    public ASFilterResponse getFilterDetailsForChile() {
        ...
        List<String> markets = accountSummaryChileRepo.findallHM();
        if (markets != null && !markets.contains(null))
            markets.sort(Comparator.naturalOrder());
        ufdr.setHomologatedName(markets);

        List<String> pNames = accountSummaryChileRepo.findallRS();
        if (pNames != null && !pNames.contains(null))
            pNames.sort(Comparator.naturalOrder());
        ufdr.setRazonSocial(pNames);

        List<String> siteNames = accountSummaryChileRepo.findallTI();
        if (siteNames != null && !siteNames.contains(null))
            siteNames.sort(Comparator.naturalOrder());
        ufdr.setTaxId(siteNames);

        List<String> allTypes = accountSummaryChileRepo.findallLocationId();
        if (allTypes != null && !allTypes.contains(null))
            allTypes.sort(Comparator.naturalOrder());
        ufdr.setLocationId(allTypes);

        List<String> mPId = accountSummaryChileRepo.findallSuppliesStatus();
        if (mPId != null && !mPId.contains(null))
            mPId.sort(Comparator.naturalOrder());
        ufdr.setSuppliesStatus(mPId);

        List<String> mpa = accountSummaryChileRepo.findAllMPA();
        if (mpa != null && !mpa.contains(null))
            mpa.sort(Comparator.naturalOrder());
        ufdr.setMpa(mpa);

        List<String> largeFormatStatus = accountSummaryChileRepo.findAllLargeFormatStatus();
        if (largeFormatStatus != null && !largeFormatStatus.contains(null))
            largeFormatStatus.sort(Comparator.naturalOrder());
        ufdr.setLargeFormatStatus(largeFormatStatus);

        List<String> hpsHardwareStatus = accountSummaryChileRepo.findAllHpsHardwareStatus();
        if (hpsHardwareStatus != null && !hpsHardwareStatus.contains(null))
            hpsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setHpsHardwareStatus(hpsHardwareStatus);

        List<String> opsHardwareStatus = accountSummaryChileRepo.findAllOpsHardwareStatus();
        if (opsHardwareStatus != null && !opsHardwareStatus.contains(null))
            opsHardwareStatus.sort(Comparator.naturalOrder());
        ufdr.setOpsHardwareStatus(opsHardwareStatus);

        List<String> bpsStatus = accountSummaryChileRepo.findAllBpsStatus();
        if (bpsStatus != null && !bpsStatus.contains(null))
            bpsStatus.sort(Comparator.naturalOrder());
        ufdr.setBpsStatus(bpsStatus);
    ...
    public String uploadRecordsForChile(MultipartFile attachment, String submittedBy, String submittedOn, String comment) throws IOException {
        ...
        for (Row row : sheet) {
            /* data.put(i, new ArrayList<String>()); */
            int i = 0;
            AccountSummaryDetailsChileDao asddao = new AccountSummaryDetailsChileDao();
            boolean repeat = false;
            AccountSummaryDetailsHistoryChileDao asdh = new AccountSummaryDetailsHistoryChileDao();
            if (k == 0) {
                boolean isValid = true;
                ...
                for (Cell cell : row) {
                    ...
                    if (cell.getColumnIndex() == 0) {
                        if (accountSummaryChileRepo.findAllRecordsByHN(getValueOf(cell)).size() != 0) {
                            repeat = true;
                            k++;
                            break;
                        }
    ...
    public AccountSummaryDetailsChileDao getRecordDetailsForChile(String homologatedName) {
        // TODO Auto-generated method stub
        return accountSummaryChileRepo.findRecordDetails(homologatedName);
    }

    @Override
    public List<AccountSummaryDetailsChileDao> getAllFilteredRecordsForChile(AccountSummaryFilterDetails searchByFields) {
        // TODO Auto-generated method stub
        boolean checkSearchStatus = false;
        List<AccountSummaryDetailsChileDao> allRevenue = new ArrayList<>();
        boolean andflag = false;
        if (!StringUtils.isEmpty(searchByFields.getHomologatedName())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo
                    .findAllRecordsLikeHN(searchByFields.getHomologatedName());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
        ...
        if (!StringUtils.isEmpty(searchByFields.getTaxId())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsLikeTaxId(searchByFields.getTaxId());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getBpsStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsByBpsStatus(searchByFields.getBpsStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getHpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsByHpsStatus(searchByFields.getHpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getOpsHardwareStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsByOpsStatus(searchByFields.getOpsHardwareStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getMpa())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsByMPA(searchByFields.getMpa());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!StringUtils.isEmpty(searchByFields.getLargeFormatStatus())) {
            checkSearchStatus = true;
            List<AccountSummaryDetailsChileDao> udd = accountSummaryChileRepo.findAllRecordsByLargeFormatStatus(searchByFields.getLargeFormatStatus());
            if (allRevenue.size() != 0) {
                andflag = true;
                allRevenue.retainAll(udd);
            } else if (!andflag) {
        ...
        if (!checkSearchStatus) {
            return getAllChileRecords();
        } else {
            return allRevenue.stream()
                    .collect(Collectors.collectingAndThen(toCollection(
                                    () -> new TreeSet<>(Comparator.comparingInt(AccountSummaryDetailsChileDao::getUnique_id))),
                            ArrayList::new));
        }
    }
    /*
...
//                log.info("STR "+ cell.getStringCellValue());
                return cell.getStringCellValue().trim();

            case Cell.CELL_TYPE_FORMULA:
//                log.info("FOR "+ cell.getCellFormula().toString());
